...
//        System.out.println(password);
        ...
        if (!Pattern.matches("[0-9]{4}", confirmPassword)  && !Pattern.matches("[0-9]{4}", password)) {
            showMessage("Pin must be 4 digits long.");
            Massage.setBackground(Color.RED);
            confirmPin.setBackground(Color.RED);
            pin.setBackground(Color.RED);
        ...
        if (!anyFieldEmpty() && passwordMatches(password, confirmPassword)) {
            ...
            String statement = String.format("Update account set pin = '%s' where customerId = '%s'", password, customer.customerID);
            sq.Update(statement);
