...
public class buyCredit  implements ActionListener  {
    ...
    ImageIcon image = new ImageIcon("src/Images/Untitled.pngg");
...
// validating user input
        ...
        if (number.length() == 11 && validatePhone(number)) {
            pass = JOptionPane.showInputDialog("Your Transaction Pin");
            if (pass.equals(user.pin)) {
                massage.setForeground(Color.BLUE);
                int n = JOptionPane.showConfirmDialog(frame, "Are you sure you want to approve this transaction of "+ amt,  "Confirm", JOptionPane.YES_NO_OPTION);
...
// checking if the user decides not to approve the transaction
    ...
    public boolean validatePhone(String phone){
        Pattern pattern = Pattern.compile("^0[7-9]{1}[0-1]{1}[0-9]{8}");
