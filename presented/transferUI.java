...
// working on the pin field
    ...
    public boolean check(){
        ...
        if (Account.isEmpty() || amt == 0 || password.length() == 0) {
            ...
            return true;
        }else if(!Pattern.matches("[0-9]{10}", Account)){
            showMessage("Account Number must be 10 digits");
        }
        else if (password.length() != 4 ) {
            showMessage("pin must be 4 digits long.");
            pin.setBackground(Color.RED);
            return true;
        }else if(!Pattern.matches("[0-9]{2,10}.0$", String.valueOf(amt))){
...
////checking if pin is valid and matches the*
// user pin in the db
    ...
    private boolean approve(String password, String account, double amt, String bank){
        ...
        if (n == JOptionPane.YES_OPTION) {
            if(bank.equals("Realm Bank")){
                if (password.equals(customer.pin) && amt < customer.balance && !customerName.equals("") && !receiverId.equals("a")) {
