...
private boolean notice(){
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (Password.getPassword().length ==8 && textField.getText().length() ==10 && e.getSource()==logIn) {
      // logic to check if password and Phone exits in DB
    }
    // constraint check on the phone input
    if (textField.getText().length() <10 ||textField.getText().length() >10 && e.getSource()==logIn ) {
      Massage.setText("Phone most be 10 digits long");
      Massage.setForeground(Color.red);
    }
    else{
      Massage.setText(null);
