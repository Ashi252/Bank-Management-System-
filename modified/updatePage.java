...
// working on the massage label
        ...
        formPanel.setBackground(homePage.color1);
...
// working on ht e panel for the company name
    ...
    private void signUp() {
        ...
        if (firstName.isEmpty() || lastName.isEmpty() || password.length == 0 || confirmPassword.length == 0 || ((String) Day).isEmpty() || ((String) Month).isEmpty() || ((String) Year).isEmpty() || ((String) Gender).isEmpty() || email.isEmpty() || address.isEmpty() ) {
...
//                bar.setString("Waiting...");
    ...
    public boolean insertData(){
        ...
        String statement = "";
        if (isPresent(userDB.ContactNumber, st)) {
            statement = String.format("insert into customer(fistName, lastName, dob, gender,address, email, password) values('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') ", firstName, lastname, dob, Gender,email, address, password);
            st.insert(statement);
            statement = String.format("select customerId from customer where contactNumber = '%s'", userDB.ContactNumber);
